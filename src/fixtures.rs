...
use crate::{Context, Metrics, Result, Topology, TopologySpec, TopologyStatus, TOPLOGY_FINALIZER};
...
impl Topology {
    ...
    pub fn illegal() -> Self {
        let mut d = Topology::new("illegal", TopologySpec::default());
    ...
    pub fn test() -> Self {
        let mut d = Topology::new("test", TopologySpec::default());
        d.meta_mut().namespace = Some("default".into());
        d
    }
    ...
    pub fn finalized(mut self) -> Self {
        self.finalizers_mut().push(TOPLOGY_FINALIZER.to_string());
        self
    }

    /// Modify a document to have an expected status
    pub fn with_status(mut self, status: TopologyStatus) -> Self {
...
impl ApiServerVerifier {
    ...
    async fn handle_finalizer_creation(mut self, doc: Topology) -> Result<Self> {
        ...
        let expected_patch = serde_json::json!([
            ...
            { "op": "add", "path": "/metadata/finalizers", "value": vec![TOPLOGY_FINALIZER] }
        ]);
    ...
    async fn handle_finalizer_removal(mut self, doc: Topology) -> Result<Self> {
        let (request, send) = self.0.next_request().await.expect("service not called");
        ...
        let expected_patch = serde_json::json!([
            { "op": "test", "path": "/metadata/finalizers/0", "value": TOPLOGY_FINALIZER },
    ...
    async fn handle_status_patch(mut self, doc: Topology) -> Result<Self> {
        let (request, send) = self.0.next_request().await.expect("service not called");
        assert_eq!(request.method(), http::Method::PATCH);
        assert_eq!(
            request.uri().to_string(),
